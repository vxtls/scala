...
/////////////////////////////////////////////////////////////////////////////////
	...
	Type bridgeType = bridgeTypeFor(owner, sym1);
        if (hasSameJvmType(bridgeType, bridgeTypeFor(owner, sym)))
            return;
    ...
    private boolean overridesAfterErasure(Type pre, Symbol overriding, Symbol overridden) {
        ...
        if (overridingType == Type.NoType || overridingType == Type.ErrorType ||
            ...
            return false;
        return overridingType.isSubType(overriddenType);
    }
    ...
    private boolean hasSameBridgeType(Symbol owner, Symbol sym1, Symbol sym2) {
        Type type1 = bridgeTypeFor(owner, sym1);
        Type type2 = bridgeTypeFor(owner, sym2);
        if (type1 == Type.NoType || type1 == Type.ErrorType ||
            type2 == Type.NoType || type2 == Type.ErrorType)
            return false;
        return hasSameJvmType(type1, type2);
    }
    ...
    private Symbol findErasedOverrideInBase(Symbol method, Type base, Symbol owner) {
        ...
        for (int i = 0; i < candidates.length; i++) {
            ...
            if (!overridesAfterErasure(site, method, candidate))
                continue;
    ...
    private Symbol findErasedOverridingMethod(Symbol owner, Symbol method) {
        ...
        for (int i = 0; i < candidates.length; i++) {
            Symbol candidate = candidates[i];
            if (!overridesAfterErasure(site, candidate, method))
                continue;
            if (!candidate.isDeferred())
                return candidate;
            if (deferred == Symbol.NONE)
    ...
    public void addBridgeMethodsTo(Symbol method) {
        ...
        Type[] parents = method.owner().parents();
        for (int i = 0; i < parents.length; i++) {
            Symbol overridden = method.overriddenSymbol(parents[i]);
            if (overridden == Symbol.NONE)
                overridden = findErasedOverrideInBase(method, parents[i], method.owner());
            if (overridden != Symbol.NONE &&
                !hasSameBridgeType(method.owner(), overridden, method))
	...
	assert owner.isClass() && !owner.isInterface(): Debug.show(owner);
        ...
        if (overriding == method) {
            ...
            if (overridden == Symbol.NONE)
                overridden = findErasedInterfaceOverride(method, owner);
            if (overridden != Symbol.NONE &&
                !hasSameBridgeType(owner, overridden, method))
                addBridge(owner, method, overridden);
        } else if (overriding != Symbol.NONE &&
                   !hasSameBridgeType(owner, overriding, method))
