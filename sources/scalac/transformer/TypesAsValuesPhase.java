...
public class TypesAsValuesPhase extends Phase {
    ...
    private boolean monoPrimitive(Symbol sym) {
        return sym.isJava()
            ...
            || isScalaRunTimeTryHelper(sym);
    }

    private boolean isScalaRunTimeTryHelper(Symbol sym) {
        Symbol owner = sym.owner();
        if (hasName(sym, Names.Try) && isScalaRunTime(owner))
            return true;
        if (hasName(sym, Names.Catch) && hasName(owner, Names.Try))
            return isScalaRunTime(owner.owner());
        return false;
    }

    private boolean hasName(Symbol sym, Name name) {
        return sym.name == name || sym.nameString().equals(name.toString());
    }

    private boolean isScalaRunTime(Symbol sym) {
        String name = sym.nameString();
        return name.equals(Names.ScalaRunTime.toString())
            || name.equals(Names.ScalaRunTime.toString() + "$");
    }
    ...
    private class TV_Transformer extends TV_MiniTransformer {
        ...
        public Tree transform(Tree tree) {
            ...
            if (tree instanceof Tree.Block) {
                ...
                return gen.Block(tree.pos,
                                 transformStatements(block.stats),
        ...
        private Tree[] transformStatements(Tree[] stats) {
            ...
            for (int i = 0; i < stats.length; ++i) {
                ...
                } else if (stat instanceof Tree.AbsTypeDef
                           || stat instanceof Tree.AliasTypeDef) {
        ...
        private boolean isSpecial(Type tp) {
            if (tp instanceof Type.TypeRef) {
                Symbol sym = ((Type.TypeRef)tp).sym;
        ...
        private Tree typeAsValue(int pos, Type tp, Symbol owner, TEnv env) {
            if (tp instanceof Type.ConstantType) {
                return typeAsValue(pos,
                                   ((Type.ConstantType)tp).base,
                                   owner,
                                   env);
            }
            if (tp instanceof Type.TypeRef) {
            ...
            if (tp instanceof Type.ThisType) {
