...
public class ATreeFromSTree {
    ...
    private ACode expression(Tree tree) {
        if (tree instanceof Tree.LabelDef) {
            ...
            if (newTree.init instanceof Tree.Apply) {
                Tree.Apply apply = (Tree.Apply)newTree.init;
                if (apply.fun instanceof Tree.Select) {
                    Tree.Select select = (Tree.Select)apply.fun;
                    if (select.qualifier instanceof Tree.Create)
                        return apply(tree, apply.fun, ((Tree.Create)select.qualifier).targs, apply.args);
                }
            }
            throw Debug.abort("illegal case", tree);
        ...
        Type[] targs, Tree[] vargs)
    {
        ...
        } else if (generator == Generator.ANYNI) {
            assert targs.length == 0 && vargs.length == 1: tree;
    ...
    private static class Generator {
        ...
        private static final Generator ANYNI =
            new Generator("ANYNI", null, null);
