...
public class GenTransformer {
    ...
    public Tree transform(Tree tree) {
        if (tree == Tree.Empty) {
            ...
            Symbol symbol = getSymbolFor(tree);
            if (symbol != null) {
                return global.make.Bind(tree.pos, symbol, rhs);
            }
            return global.make.Bind(tree.pos, node.name, rhs);
            ...
            Switch result = global.make.Switch(tree.pos, test, node.tags, bodies, otherwise);
            result.setType(transform(tree.getType()));
            return result;
            ...
            Symbol symbol = getSymbolFor(tree);
            return gen.Create(tree.pos, transform(node.qualifier), symbol, transform(node.targs));
            ...
            if (node.args.length == 1 &&
                node.args[0] instanceof Visitor &&
                TreeInfo.methSymbol(node.fun) == global.definitions.ANY_MATCH) {
