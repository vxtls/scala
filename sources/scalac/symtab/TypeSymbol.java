...
public abstract class TypeSymbol extends Symbol {
    ...
    private void computeClosureAt(Phase phase) {
        if (closures != null && closures.start == phase)
            return;
        Phase current = Global.instance.currentPhase;
        ClosureIntervalList savedClosures = closures;
        closures = new ClosureIntervalList(closures, Type.EMPTY_ARRAY, phase);
        try {
    ...
    private static Map inclClosure(Map closure, Type type) {
        ...
        if (type == Type.ErrorType || type == Type.NoType) {
            ...
            return inclClosure(closure, type.singleDeref());
        }
