    ...
    static class AsSeenFromMap extends Map {
        ...
        public Type apply(Type type) {
            ...
            if (type instanceof ThisType) {
                ...
                if (typeRef0.sym.owner().isPrimaryConstructor()) {
                    assert typeRef0.sym.kind == TYPE;
                    return type.toInstance(typeRef0.sym, pre, clazz);
                }
                return map(type);
...
*/
	...
	*/
        if (leastBaseTypes.length == 1 && members.isEmpty()) {
            Type candidate = leastBaseTypes[0];
            Symbol candidateSym = candidate.symbol();
            if (candidateSym.isInterface() && (candidateSym.flags & JAVA) != 0) {
                boolean hasValueClass = false;
                boolean hasNonValueClass = false;
                for (int i = 0; i < tps.length; i++) {
                    Symbol sym = tps[i].widen().symbol();
                    if (sym.isSubClass(Global.instance.definitions.ANYVAL_CLASS)) {
                        hasValueClass = true;
                    } else {
                        hasNonValueClass = true;
                    }
                }
                if (hasValueClass && hasNonValueClass) {
                    return Global.instance.definitions.ANY_TYPE();
                }
            }
            return candidate;
