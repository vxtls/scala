    ...
    private static class ThisTypeMap extends Map {
        ...
        private void initialize(Symbol clasz, Type type) {
            ...
            if (base instanceof TypeRef) {
                ...
                assert typeRef.sym == clasz && typeRef.args.length == params.length:
                    type + "@" + Debug.show(clasz) + " -> " + base;
                for (int i = 0; i < params.length; i++) {
                    assert params[i].isParameter(): Debug.show(params[i]);
                    subst.put(params[i], typeRef.args[i]);
                }
                initialize(clasz.owner(), typeRef.pre);
                return;
            }
        ...
        public Type apply(Type type) {
            if (type instanceof ThisType) {
                ...
                if (lookup == null) return map(type);
                return lookup;
            } else if (type instanceof TypeRef) {
                TypeRef typeRef = (TypeRef)type;
                if (!typeRef.sym.isParameter()) return map(type);
                assert typeRef.args.length == 0: type;
                Type lookup = (Type)subst.get(typeRef.sym);
                if (lookup == null) return map(type);
                return lookup;
            }
            return map(type);
