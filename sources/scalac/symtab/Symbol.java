...
// Symbol classification ----------------------------------------------------
    ...
    public final boolean isInitializedMethod() {
        ...
        if (rawInfo instanceof Type.MethodType || rawInfo instanceof Type.PolyType) {
            return true;
    ...
    public final boolean isOverloaded() {
        Type tp = rawInfo();
        return tp instanceof Type.OverloadedType ||
...
// Symbol origin --------------------------------------------------------------
    ...
    public Symbol getMainOwner() {
        if (isModule()) return moduleClass();
        if (isConstructor()) return constructorClass();
        if (isCaseFactory()) return type().resultType().symbol();
        return owner();
    }

    public Symbol getMainClassOrNone() {
        if (isNone() || (isClassType() && !isCompoundSym())) return this;
    ...
    public final AbstractFile getSourceDirectoryOrNull() {
        ...
        SymbolOrigin origin = getOrigin();
        if (origin instanceof SymbolOrigin.Directory)
            return ((SymbolOrigin.Directory)origin).file;
    ...
    public final AbstractFile getSourceFileOrNull() {
        SymbolOrigin origin = getOrigin();
        if (origin instanceof SymbolOrigin.ClassFile) {
            String sourcefile = ((SymbolOrigin.ClassFile)origin).sourcefile;
            if (sourcefile == null) return null;
            AbstractFile parent = getSourceDirectoryOrNull();
            return parent == null ? null : parent.lookupName(sourcefile, false);
        }
        if (origin instanceof SymbolOrigin.ScalaFile)
            return ((SymbolOrigin.ScalaFile)origin).file;
        if (origin instanceof SymbolOrigin.ScalaUnit)
            return ((SymbolOrigin.ScalaUnit)origin).unit.source.getFile();
        return null;
    }
    ...
    public final CompilationUnit getUnitOrNull() {
        SymbolOrigin origin = getOrigin();
        if (origin instanceof SymbolOrigin.ScalaUnit)
            return ((SymbolOrigin.ScalaUnit)origin).unit;
        return null;
    }
...
// Acess to related symbols -----------------------------------------------------
    ...
    /** Get accessor for a case class field. */
    ...
    public Symbol stableAlternative() {
        ...
        if (tp instanceof Type.OverloadedType) {
            Symbol[] alts = ((Type.OverloadedType)tp).alts;
            for (int i = 0; i < alts.length; i++)
...
// Symbol types --------------------------------------------------------------
    ...
    // where
		...
		boolean altChanged = false;
		    ...
		    if (type != alttypes[i]) altChanged = true;
		}
		Type result;
		if (n < alts.length) {
			...
			if (keepInheritedOverloaded ||
			    ...
			    types[j] = alts[i].info();
			    j++;
			} else if (Global.instance.debug) {
			    Global.instance.log("removing inherited alternative " + alts[i] + ":" + alttypes[i]);
			}
		    }
...
// Overloading and Overriding -------------------------------------------
    ...
    public void addInheritedOverloaded(Type owntype) {
        ...
        if (sym.kind == VAL) {
            ...
            if (symtype instanceof Type.OverloadedType) {
                Type.OverloadedType overloadedType = (Type.OverloadedType)symtype;
                Symbol[] alts = overloadedType.alts;
                Type[] alttypes = overloadedType.alttypes;
                for (int i = 0; i < alts.length; i++)
    ...
    public void addInheritedOverloaded(Type owntype, Symbol sym, Type symtype) {
        if (!owntype.overrides(symtype)) {
            if (Global.instance.debug) {
                Global.instance.log(owner() + " inherits overloaded: " + sym + ":" + symtype + sym.locationString());
            }
    ...
    public Type removeInheritedOverloaded(Type owntype) {
        if (owntype instanceof Type.OverloadedType) {
            Type.OverloadedType overloadedType = (Type.OverloadedType)owntype;
            Symbol[] alts = overloadedType.alts;
            Type[] alttypes = overloadedType.alttypes;
            int n = 0;
            for (int i = 0; i < alts.length; i++) {
            ...
            if (n < alts.length) {
                ...
                for (int i = 0; i < alts.length; i++) {
                    if (alts[i].owner() == owner()) {
                        ...
                        Global.instance.log("removing inherited alternatve " + alts[i] + ":" + alttypes[i]);
                    }
                }
