...
public class ClassfileParser implements ClassfileConstants {
    ...
    protected void parse(int flags) {
        {
            ...
            staticsClass.flags &= ~Modifiers.INITIALIZED;
            ...
            int savedStaticsFlags = staticsClass.flags;
            c.flags |= Modifiers.INITIALIZED;
            staticsClass.flags |= Modifiers.INITIALIZED;
            ...
            //    }
            //}
            c.flags = savedFlags | Modifiers.INITIALIZED;
            staticsClass.flags = savedStaticsFlags | Modifiers.INITIALIZED;
    ...
    protected void parseMethod() {
        ...
        if ((sflags & Modifiers.PRIVATE) != 0) {
            ...
            if (name != CONSTR_N) {
                // Primitive foundation classes encode Pico-style nullary coercions
                // as distinct JVM method names; after remapping them back to
                // `coerce`, they still need to participate in overload lookup.
                getScope(jflags).enterOrOverload(symbol);
    ...
    private Type foundationNullaryResult(Name name) {
        ...
        if (fullname.equals("scala.Float")) {
            ...
            if (name == Names.toByte)
                return definitions.BYTE_CLASS.typeConstructor();
            if (name == Names.toShort)
                return definitions.SHORT_CLASS.typeConstructor();
            if (name == Names.toChar)
                return definitions.CHAR_CLASS.typeConstructor();
            if (name == Names.toInt)
                return definitions.INT_CLASS.typeConstructor();
            if (name == Names.toLong)
                return definitions.LONG_CLASS.typeConstructor();
            if (name == Names.toFloat)
                return definitions.FLOAT_CLASS.typeConstructor();
            if (name == Names.toDouble)
