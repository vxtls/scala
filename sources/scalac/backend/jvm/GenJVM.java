    ...
    protected JType genLoad(Context ctx, Tree tree, JType expectedType)
        ...
        if (tree instanceof Tree.LabelDef) {
            ...
            generatedType = new JObjectType(className);
                   ...
                   && (((Tree.TypeApply)((Tree.Apply)tree).fun).fun.symbol() == defs.ANY_IS
                       || ((Tree.TypeApply)((Tree.Apply)tree).fun).fun.symbol() == defs.ANY_AS)) {
            ...
            if (sym.isModule())
                ...
                if (isStaticMember(sym))
                    ctx.code.emitGETSTATIC(javaName(sym.owner()), jvmName(ident.name), type);
                else {
                    ctx.code.emitALOAD_0();
                    ctx.code.emitGETFIELD(ctx.clazz.getName(), jvmName(ident.name), type);
                }
                generatedType = type;
            } else {
            ...
            if (sym.isModule())
                ...
                if (MODULE_INSTANCE_FIELD_NAME.equals(fieldName)) {
                    Symbol owner = sym.owner();
                    Symbol moduleClass = owner;
                    if (!moduleClass.isModuleClass()
                        && owner.isClassType()
                        && owner.linkedModule() != null
                        && owner.linkedModule().isModule()) {
                        moduleClass = owner.linkedModule().moduleClass();
                    }
                    if (moduleClass.isModuleClass()) {
                        className = javaName(moduleClass);
                        fieldType = new JObjectType(className);
