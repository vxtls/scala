...
public class Evaluator {
    ...
    private Object evaluate(Code code) {
        ...
        if (code instanceof Code.Create) {
            ...
            return invoke(null, template.getConstructor(), new Object[] {template.getHandler()});
        }
        if (code instanceof Code.IsScala) {
            Code.IsScala isScala = (Code.IsScala)code;
            Object object = evaluate(isScala.target);
            if (object == null || !isScalaObject(object)) return Boolean.FALSE;
            Symbol actual = getScalaObject(object).template.getSymbol();
            return new Boolean(actual.isSubClass(isScala.symbol));
        }
        if (code instanceof Code.IsJava) {
            Code.IsJava isJava = (Code.IsJava)code;
            Object object = evaluate(isJava.target);
            return new Boolean(isJava.clasz.isInstance(object));
        }
