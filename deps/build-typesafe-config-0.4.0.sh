...
work_dir="$stage_dir/build/source-deps/typesafe-config-0.4.0"
src_dir="$work_dir/src"
classes_dir="$work_dir/classes"
sources_jar="$work_dir/config-0.4.0-sources.jar"
out_dir="$stage_dir/lib/extra"
out_jar="$out_dir/typesafe-config-0.4.0.jar"
...
if [[ ! -f "$sources_jar" ]]; then
  curl -fsSL "$repo_url/com/typesafe/config/0.4.0/config-0.4.0-sources.jar" -o "$sources_jar"
fi
